...
import { FinanceStateService, JournalLine } from '../services/finance-state.service';
...
export class JournalEntryComponent implements OnInit {
  ...
  entries         = this.financeState.journalEntries;
  lines           = this.financeState.journalLines;
  ...
  onSave(data: JournalFormData & { status: string }): void {
    ...
    this.http.post<JournalEntry>(
      ...
      next: (created) => {
        // Save journal lines
        const lineRequests = data.lines.map((line, i) => {
          const linePayload = {
            id:               `jl-${Date.now()}-${i}`,
          ...
          return this.http.post(API_ENDPOINTS.JOURNAL_LINES, linePayload);
        });
        ...
        if (lineRequests.length > 0) {
          forkJoin(lineRequests).subscribe(() => {
            ...
            this.notification.success(`Journal entry ${ref} created`);
          });
        } else {
          this.financeState.loadState();
          this.showForm.set(false);
